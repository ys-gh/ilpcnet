...
class LPClite(object):
    ...
    def __init__(self, fl=320, fs=80, order=29, window='blackman', flag_emph=True, emph_coef=0.97):
    ...
    def _auto_correlation(self, framed_x):
        ...
        for i in np.arange(self.order + 1):
            autocor[:, i] = np.sum(framed_x[:, 0:self.fl - i] * framed_x[:, i:], axis=1)
        #     print(autocor[0, i])
        #     autocor[:, i] = 0
...
#     # LPC analysis
#     lpc_coef, _, rc, gain, err, err_overlapped = m_lpc.analysis(np.expand_dims(data1, axis=1))
...
#     plot_API.plot_API([wav_re[:, 0], data1, err_overlapped[:, 0], excitation_new[:, 0]],
#                       plot_lib.plot_spec, 'v')

#     plot_API.plot_API([wav_re[:, 0] - err_overlapped[:, 0]], plot_lib.plot_spec, 'single')
